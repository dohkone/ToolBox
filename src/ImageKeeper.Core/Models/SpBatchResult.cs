...
public sealed class SpBatchResult
{
	...
	public int SuccessCount => Results.Count((SpBatchJobResult item) => string.Equals(item.Status, "generated", StringComparison.OrdinalIgnoreCase));

	public int SkippedCount => Results.Count((SpBatchJobResult item) => string.Equals(item.Status, "skipped", StringComparison.OrdinalIgnoreCase));

	public int FailedCount => Results.Count((SpBatchJobResult item) => string.Equals(item.Status, "failed", StringComparison.OrdinalIgnoreCase));
}
